...
| Versioned C-like scalar return-expression patch and bounded x86-64 lowering | IRENE-3's edited-C patch-region concept used as a design reference; no upstream parser/source copied | `hydir-patch`, `hydir-cli::patch`, `hydir-server::apply_patch` | Newly implemented, whole-function/entry-only subset, not PatchLang compatibility | Four intentional local behavior cases, size/hash/overwrite refusals; remote idempotent revision/restart/isolation demo |
...
| Named LLVM pass experiment | LLVM `opt` 14.0.6 command line, separately installed in the pinned Linux image | `hydir-transform`, `hydir-cli::passes`, `hydir-server::transform` | Newly implemented allowlisted orchestration, not an LLVM port | LLVM verification and eight transformed trusted-fixture boundary cases; remote artifact/restart/owner-isolation demo |
| Restricted whole-executable LLVM reconstruction and guest-memory/syscall runtime | `object` 0.39.1, `iced-x86` 1.21.0, external Clang/LLVM 14.0.6; no copied upstream implementation | `hydir-recompile`, `native/whole-runtime`; thin CLI and server callers | Newly implemented freestanding subset, not a Remill/Anvill/Rellic port | Three locally rebuilt complete ELFs/five controlled matches; one remote rebuilt ELF/three client-side matches, revision/restart/isolation checks, five rejected local cases, LLVM verification |
| Versioned gRPC schema and client/server | `tonic`/`prost` 0.14.6/0.14.4 | `hydir-api`, `hydir-cli`, `hydir-server` | Newly implemented integration; local-only subset | Separate-process remote demo including C artifact and protocol unit tests |
...
| Desktop workbench | `eframe`/`egui` 0.36.2 | `hydir-gui` | Newly implemented local and explicit remote inspection, pass, scalar patch, and rebuild subset | macOS populated-window render/initial-selection smoke; Linux local/remote operation probes for passes, patching, rebuild/export; direct visual interaction QA pending |
| Python API client and generated stubs | First-party `hydir.proto`; `grpcio-tools` 1.84.0 generator | `sdk/python/hydir_sdk` | Newly implemented wrapper and generated protocol bindings | Seven Python boundary tests; separate-process smoke and remote rebuild with three client-side behavior comparisons; pass/source/C/patch wrappers |
