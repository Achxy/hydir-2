...
- A local `hydird` service, project ID/revision, and private token file.
...
inside a function, enter the loopback endpoint, token file, project ID, and
current revision. The workflow is compile/verify preview first, then apply to a
new ELF path. Existing output files are refused by `hydirctl`.
