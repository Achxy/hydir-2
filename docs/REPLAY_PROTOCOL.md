...
Capture argv remains limited to ASCII letters, digits, `_`, `-`, `.`, and `/`; replay accepts the broader raw-byte argv contract. Paths whose first component starts `.hydir-` are reserved for runner staging. A stop at a function entry or address does not by itself establish a mapping from parsed state back to original input bytes. The Linux CI smoke test is configured to require both named-symbol capture and a stripped-PIE stop inside a validator after `fgets`; it checks that the captured argument bytes equal the declared stdin origin. This is a test oracle, not an automatic provenance claim. It has not been run on this Windows host. `doctor` reports GDB availability and keeps `gdb_capture_v1` false until that gate and the broader capture workflow pass. Product-level post-input origin mapping, complete TLS state, demand memory reads, Triton resumption, and richer process events remain open.
