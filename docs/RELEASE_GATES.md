...
| --- | --- | --- |
| M0 baseline | Rust 1.96.0, Cargo lockfile, Debian Clang/LLVM 14.0.6 image, exact upstream revision and scoped file-level license-exception inventory | Complete distribution-level dependency/notice and legal audit, one compatible Remill/Anvill/Rellic/MLIR set, upstream reference lift |
| M1 native vertical slice | Ghidra-free ELF import and scalar CFG-to-LLVM lift; v2 ELF program inventory with load mappings, imports, relocations, and separate partial analyzed-spec call/reference overlay; 20 distinct supported functions, 21,168/21,168 tested input pairs across 21 variants; explicit unsupported refusal | Stack/buffer and partial-register semantics, optimization-level diversity, broader ABI and binary corpus; complete call/reference recovery |
| M2 shared product | egui local-project window visually inspected on macOS; private local path-bound SQLite annotation ledger and saved pane widths/recent ELF with explicit reopen, shared by CLI/GUI, explicit remote upload/pass/rebuild/annotation headless probes, owner-scoped remote SQLite revisions/artifacts and analyst facts, authenticated loopback gRPC, durable lift jobs, replay/cancel, Python SDK subset | Broader workspace/session restoration, reliable visual interaction QA, full typed operation parity and local/remote project sync, fine-grained roles, quotas/audit, non-loopback TLS, hostile-input isolation |
