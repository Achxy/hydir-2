...
| M2 shared product | egui local-project window visually inspected on macOS; private local path-bound SQLite annotation ledger shared by CLI/GUI, explicit remote upload/pass/rebuild/annotation headless probes, owner-scoped remote SQLite revisions/artifacts and analyst facts, authenticated loopback gRPC, durable lift jobs, replay/cancel, Python SDK subset | Saved/reopened layouts, reliable visual interaction QA, full typed operation parity and local/remote project sync, fine-grained roles, quotas/audit, non-loopback TLS, hostile-input isolation |
