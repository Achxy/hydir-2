...
**Status claim (2026-09-26):** a thin headless Ghidra exporter and Rust importer produce a binary-bound raw P-code artifact with analyzed flow and call evidence from a real x86-64 ELF. The CLI launches a bounded worker automatically, and the GUI opens an ELF, imports the function index, and browses source-linked raw P-code, state effects, flow, calls, and individual LLVM operations. The default worker has a pinned Docker image definition; its container route is untested on this host because Docker is unavailable. A local Ghidra 12.1.4 route has passed an end-to-end fixture and cache check, including reuse of a saved analyzed project for another selected function. Rust classifies a limited pure integer P-code subset, emits an ordered state-effect inventory and an incomplete instruction CFG artifact, and emits verifier-clean LLVM for an individual exact operation. The Python SDK has a local CLI-backed route to these artifacts. Executable function state/CFG semantics, full-function LLVM, remote API exposure of this new route, and the later analysis and rewrite gates remain open. The previous estimate that roughly 60% of the *typed native decompilation* milestone was done does not measure progress on this revised product.
