...
**Status claim (2026-09-26):** a thin headless Ghidra exporter and Rust importer produce a binary-bound raw P-code artifact from a real x86-64 ELF. The CLI now launches a bounded worker automatically, and the GUI opens an ELF, imports the function index, and browses raw P-code with source addresses and diagnostics. The default worker has a pinned Docker image definition; its container route is untested on this host because Docker is unavailable. A local Ghidra 12.1.4 route has passed an end-to-end fixture and cache check. Rust now classifies a limited pure integer P-code subset and emits verifier-clean LLVM for an individual exact operation. Project persistence, complete function state/CFG semantics, full-function LLVM, API/SDK exposure of this new route, and the later analysis and rewrite gates remain open. The previous estimate that roughly 60% of the *typed native decompilation* milestone was done does not measure progress on this revised product.
...
The exporter, Rust P-code artifact, automatic CLI worker, and GUI raw P-code view are in place. The current CLI command is `hydirctl ghidra analyze`. Snapshot persistence in `hydir-project`, a durable analyzed Ghidra project, and the function-level semantic/LLVM path are the next ownership steps. Preserve `HydIRExport.java` and the v1 graph reader. Continue with `hydir_max2` and PRISM ELF fixtures before adding new binaries.
