...
| 3. LLVM | First compare Patchestry as an external backend on Hydir fixtures, then integrate it if its outputs preserve the required provenance and semantics or lower the supported P-code subset through Hydir IR | `opt -verify` plus behavioral comparison against original function and a Ghidra P-code oracle on exact fixtures; failed comparison removes the exact claim |
...
| Headless container and rich Ghidra JSON | Study [Patchestry's exporter](https://github.com/lifting-bits/patchestry/blob/main/scripts/ghidra/PatchestryDecompileFunctions.java), [serializer](https://github.com/lifting-bits/patchestry/blob/main/scripts/ghidra/util/PcodeSerializer.java), and [container](https://github.com/lifting-bits/patchestry/blob/main/scripts/ghidra/decompile-headless.dockerfile); test its output on a Hydir fixture | Patchestry already has a Ghidra→P-code JSON→MLIR/CIR→LLVM path. Its serializer operates on high P-code and adds synthetic operations, so its JSON cannot silently become Hydir's raw-semantic source. Adapt relevant metadata/type serialization after a fixture-level comparison; keep Hydir's snapshot schema and importer. Its patched LLVM/MLIR toolchain has a substantial build and packaging cost that the backend bake-off must justify. |
...
| LLVM translation | Read [Ghidrall](https://github.com/toor-de-force/Ghidrall), [Ghidra-to-LLVM](https://github.com/toor-de-force/Ghidra-to-LLVM), [Patchestry](https://github.com/lifting-bits/patchestry), and [Remill](https://github.com/lifting-bits/remill) for opcode and memory-model choices | These projects prove the idea exists. Ghidra-to-LLVM documents Ghidra 9.1.1 and only optimization level 0; Ghidrall requires radare2; Patchestry uses high P-code plus a patched LLVM/MLIR stack; Remill starts from machine code. Do a compatibility spike before importing any code. Hydir's state/effect and fidelity contracts remain the integration target. |
...
1. Export a deterministic program index and one raw-P-code function snapshot from a new `HydIRSnapshot.java` beside the existing v1 graph exporter, using the pinned Ghidra release. Inspect Patchestry's JSON on the same fixture and reuse only fields or serializers that preserve Hydir's raw/high distinction and provenance. Check in the fixture and an invalid-snapshot set.
...
4. Run a backend bake-off on `hydir_max2`, a PRISM branch, and one memory/call fixture: feed the same Ghidra analysis to Patchestry and Hydir's snapshot, record opcode coverage, address provenance, executable LLVM behavior, build cost, and missing effects. If Patchestry passes the semantics and packaging gate, integrate it as an explicitly named external LLVM backend. Otherwise lower a documented x86-64 P-code subset through Hydir's state IR. In either case compare bounded execution against Ghidra P-code evaluation and native ELF runs, show an exact/partial ledger, and add one meaningful pass and checked rewrite only after comparisons pass.
