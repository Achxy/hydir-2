...
**Next implementation action:** translate general ExpressionIR flag predicates through joins, extend CFG layout annotations to derived pointers and arrays of structures, then add bounded call effects and the remaining supported widths while preserving every unsupported effect. **First new execution payoff:** capture an input boundary, explain a failed condition, and replay a working input against the original binary.
