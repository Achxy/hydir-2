...
| Typed C | Linear supported 64-bit operations, frame spills, fields, fixed direct calls; `ExpressionIR v1` now preserves SSA joins, source bytes, and residual effects, and normalizes supported scalar writes | Flags, memory, calls, broader widths, branches, loops, indexed memory, supported indirect calls |
...
**Next implementation action:** complete ExpressionIR semantics for supported widths, flags, memory, and calls while preserving every unsupported effect. **First new execution payoff:** capture an input boundary, explain a failed condition, and replay a working input against the original binary.
