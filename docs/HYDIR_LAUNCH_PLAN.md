...
| Typed C and expression layer | `HighLevelCIR v1` handles linear supported 64-bit operations, frame spills, fields, and fixed direct calls. `HighLevelCfgCir v2` adds a separate exact, memory-free scalar subset for direct branches, joins, and loops; its renderer structures private diamonds and simple pre-test loops, retaining gotos elsewhere. Scalar writes and live zero-flag snapshots lower from `ExpressionIR v1`; zero/nonzero branches after supported arithmetic now render as C11. ExpressionIR preserves SSA joins, source bytes, and residual effects and normalizes supported scalar writes, condition flags, branch predicates, little-endian MOV loads/stores with alias-region dependencies, and LEA | Finish general flag/control translation through ExpressionIR, structure nested/complex regions, add typed memory and calls across joins, full widths, indirect calls, and selective caching for v2 |
...
The integrated branch passes `cargo test --workspace --locked`; the Triton bridge, instruction oracle, SDK boundary, CLI doctor, model verification, and ExpressionIR smoke checks also passed in this development environment. These tests cover bounded behavior and do not establish launch readiness. GCC and WSL2 are absent on the current Windows host, so Linux runner and dual-compiler gates require another environment. Replace any approximate completion percentage with this capability matrix and the gates below.
...
**Next implementation action:** connect ExpressionIR predicates and memory expressions to typed branches and loops, then add bounded call effects and remaining supported memory operations while preserving every unsupported effect. **First new execution payoff:** capture an input boundary, explain a failed condition, and replay a working input against the original binary.
