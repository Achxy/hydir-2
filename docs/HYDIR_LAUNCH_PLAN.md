...
| Typed C and expression layer | Typed C handles linear supported 64-bit operations, frame spills, fields, and fixed direct calls. `ExpressionIR v1` preserves SSA joins, source bytes, and residual effects; it normalizes supported scalar writes, condition flags, branch predicates, little-endian MOV loads/stores with alias-region dependencies, and LEA | Connect expressions to typed branches and loops; add memory arithmetic, calls, segment/address-size and relocation handling, broader instructions, and supported indirect calls |
