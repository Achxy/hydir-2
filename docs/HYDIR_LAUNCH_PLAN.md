...
**Status claim:** a thin headless Ghidra exporter and Rust importer now produce a binary-bound raw P-code artifact from a real x86-64 ELF. Automatic Ghidra orchestration, project persistence, semantic P-code lowering, LLVM emission from P-code, and GUI navigation remain open. The previous estimate that roughly 60% of the *typed native decompilation* milestone was done does not measure progress on this revised product.
...
The smallest code ownership change is a new `integrations/ghidra/HydIRSnapshot.java` exporter, a versioned P-code module in `hydir-ir`, snapshot persistence in `hydir-project`, and a `hydirctl analyze` worker command consumed by the GUI's existing background task mechanism. Preserve `HydIRExport.java` and the v1 graph reader. Start with the checked-in `hydir_max2` and PRISM ELF fixtures before adding new binaries.
