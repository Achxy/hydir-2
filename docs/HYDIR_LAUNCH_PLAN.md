...
| P0c. Ship the beta | Install, complete the investigation without a custom script, export/reproduce the recipe, and compare fairly | M8 beta usability, packaging, and held-out challenges | A bounded CLI recipe and claim pass export, verification, and fresh replay; GUI flow, clean installs, held-out challenges, and comparisons remain |
...
| Launch branch | `hydir-launch` integrates the typed/model/VM foundation, `ExpressionIR v1`, typed CFG subsets, bounded native capture/replay, trace-specific slice, and CLI investigation recipe | Finish M0 GUI/package gates and extend M1 semantics |
