...
| Execution foundation | `InputSpec v1`, bounded Bubblewrap replay, `NativeReplayReport v1`, `ExecutionSnapshot v1`, sparse reads, and experimental named-symbol/stripped-address GDB/MI capture | Live Linux capture gate, post-input stop, original-byte origin mapping, Triton resumption, cancellation, WSL2 launch |
...
The bounded GDB/MI record parser handles result, asynchronous, and stream records with nested values. An experimental local session controller stops a single-threaded process at a named function or a relocated file-backed ELF address, records mappings and selected pages, and emits a validated snapshot. It disables GDB init files and auto-loading before the ELF is loaded. The Linux smoke gate now includes a stripped PIE stop inside a validator after `fgets` and checks captured argument bytes against the declared stdin origin. This fixture equality is only a test oracle; automatic origin mapping remains open. Cross-compilation and parser checks pass on Windows; the Linux smoke gate must demonstrate replay and both capture paths before the capability claim changes. `doctor` keeps `gdb_capture_v1` false pending that gate.
