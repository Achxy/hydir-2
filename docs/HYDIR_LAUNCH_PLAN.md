...
| Typed C and expression layer | `HighLevelCIR v1` handles linear supported 64-bit operations, frame spills, fields, and fixed direct calls. `HighLevelCfgCir v2` adds a separate exact, memory-free scalar subset for direct branches, joins, and loops; its renderer structures private diamonds and simple pre-test loops, retaining gotos elsewhere. Its scalar register writes now lower from freshly computed `ExpressionIR v1`, which preserves SSA joins, source bytes, and residual effects and normalizes supported scalar writes, condition flags, branch predicates, little-endian MOV loads/stores with alias-region dependencies, and LEA | Finish flag/control translation through ExpressionIR, structure nested/complex regions, add typed memory and calls across joins, full widths, indirect calls, and selective caching for v2 |
