...
**Next implementation action:** extend struct-array and pointer annotations beyond entry-block derivations, add bounded call effects and the remaining supported widths, then connect the expression layer to capture and replay while preserving every unsupported effect. **First new execution payoff:** capture an input boundary, explain a failed condition, and replay a working input against the original binary.
