...
| Triton integration | Bounded function-level symbolic bridge with two symbolic argument registers plus an experimental captured-state pure-validator return solve. Local Triton tests cover a direct goal, branch flip, missing state, and budget exhaustion | Live Linux capture → solve → replay gate, broader goals, external summaries, and guided UI |
| Execution foundation | `InputSpec v1`, bounded Bubblewrap replay, `NativeReplayReport v1`, `ExecutionSnapshot v1`, sparse reads, `OriginProbe v1` byte-equality checks, `SnapshotResumePlan v1`, and experimental named-symbol/stripped-address GDB/MI capture | Live Linux capture gate, automatic original-byte provenance, cancellation, WSL2 launch |
...
The bounded GDB/MI record parser handles result, asynchronous, and stream records with nested values. An experimental local session controller stops a single-threaded process at a named function or a relocated file-backed ELF address, records mappings and selected pages, and emits a validated snapshot. It disables GDB init files and auto-loading before the ELF is loaded. `OriginProbe v1` now compares a declared input origin to memory at an analyst-selected captured register and records only byte equality. The Linux smoke gate includes a stripped PIE stop inside a validator after `fgets` and requires that probe to match the declared stdin bytes. Automatic origin provenance remains open. Cross-compilation and parser checks pass on Windows; the Linux smoke gate must demonstrate replay and both capture paths before the capability claim changes. `doctor` keeps `gdb_capture_v1` false pending that gate.
