...
| Triton integration | Bounded function-level symbolic bridge with two symbolic argument registers plus an experimental captured-state pure-validator return solve. Local Triton tests and one Linux capture → solve → original-ELF replay gate passed | Broader goals, external summaries, input/condition slices, and guided UI |
