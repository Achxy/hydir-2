...
docker run --rm --platform linux/amd64 \
  ...
  hydir-m1:local bash -c 'rustc --version && clang --version | head -1 && cargo test --locked --workspace && cargo clippy --locked --workspace --all-targets -- -D warnings && bash scripts/demo-local.sh && bash scripts/demo-analysis.sh && bash scripts/demo-remote.sh'
