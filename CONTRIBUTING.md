...
The CI check validates the HTML and local links/assets. It runs without deployment secrets. Do not use `vercel --prod` for ordinary changes: the Git integration handles production releases. Preview URLs currently require Vercel authentication; reviewers need access or an owner-shared link. If a private-repository contributor's deployment is denied, the project owner must resolve the Vercel access requirement before merge.
